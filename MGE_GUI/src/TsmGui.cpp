...
void TsmGui::TickScreen(BaseScreen& screen, double delta)
{
...
void TsmGui::tickWidgets(const std::vector<std::shared_ptr<Widget>>& widgets)
{
...
SCREEN_EVENTS& TsmGui::getScreenEventQue()
{
    return screenEvents;
}

void TsmGui::sendLmbEventToGui(const IPoint& clickPos, bool clicked)
{
...
void TsmGui::sendRmbEventToGui(const IPoint& cursorPos, bool clicked)
{
    if (clicked && actualBlockWidget)
    {
        if (const auto& button = std::dynamic_pointer_cast<Button>(actualBlockWidget))
...
bool TsmGui::isCursorBlockedByGui() const noexcept
{
    return (actualBlockWidget.use_count() != 0);
}

bool TsmGui::sendGuiCollision
