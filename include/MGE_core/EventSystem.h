...
public:
    ...
    [[nodiscard]] const T& get() const noexcept
    {
        ...
        return *ptr; //if no ¨data, ptr = nullptr"
    }
    ...
    void publishEvent(const EventDataHandler& e)
    {
        ...
        for (auto& sub : EventSystemCallbacks)
        {
            if (sub->m_token.isAlive())
            {
                try
                {
                    sub->m_EventSystemCallback(e); //could return nullptr if no data is avalaible in the event (valid state)
                }
#ifdef _DEBUG
...
public:
    ...
    void addToken(Function&& f)
    {
        Callback cb =
            [func = std::forward<Function>(f)](const EventDataHandler& e)
            {
                func(e.get<Event>());
            };
